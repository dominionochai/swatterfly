...
class SyntheticReplay:
    """Produce made-up telemetry for UI development. It is not detection or flight math."""

    def __init__(self, settings: Settings):
        self.settings = settings

    def build(self) -> ReplayFile:
        """Build deterministic frames without mutating process-wide random state."""
        ...
        for index in range(self.settings.replay_frames):
            ...
            theta = round(2.5 + 39.0 * phase + rng.uniform(-0.35, 0.35), 2)
            ...
            confidence = round(min(0.99, 0.42 + phase * 0.5 + float(np_rng.uniform(-0.03, 0.03))), 2)
            frames.append(TelemetryFrame(
                frame_id=f"{replay_id}-{index:04d}", replay_id=replay_id, theta=theta, eta=eta, tau=tau,
                target_lock_state=state, confidence=confidence,
                timestamp=start + timedelta(seconds=index * self.settings.replay_interval_seconds),
                ...
                threat_log=[f"{(start + timedelta(seconds=index * self.settings.replay_interval_seconds)).strftime('%H:%M:%S')} synthetic frame"],
